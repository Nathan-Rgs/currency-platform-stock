...
def get_summary(
    ...
    total_countries = db.scalar(select(func.count(func.distinct(Coin.country))).select_from(q.subquery())) or 0
    ...
    total_estimated_value = db.scalar(select(func.sum(Coin.estimated_value)).select_from(q.subquery())) or 0.0
